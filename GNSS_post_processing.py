...
def KMF(df, nbc_min=1, nbc_max=10, nb_cycles=5, standardisation=False, 
    ...
    def variables_importance(data_clustered):
        ...
        plt.figure()
        plt.semilogy(np.arange(0, len(data_clustered.columns)), fvalues, 
                     alpha=0.2, color='black', LineStyle='--', marker='o')
    ...
    def velocity(data_filtered):
        ...
        deltatime = pd.DataFrame(deltatime)
        deltatime_dec = deltatime.iloc[:, 0] / timedelta(days=1)
        KMF_velocity = (np.sqrt(np.diff(data_filtered.X) ** 2 
                                + np.diff(data_filtered.Y) ** 2)) / deltatime_dec
    ...
    def KMF_correction(data_filtered, nbclusters):
        '''
        In Adrien Wehrlé's work, the data remained under-processed with the 
        initial KMF method due to a high number of outliers.
        It is therefore proposed to run the algorithm nb_cycles times to increase 
        the precision of the filtering. Once more, the optimal number of cycles 
        is determined with the help of the Elbow Method.
        '''
        
        data_filtered_interm, c, l = K_means(data_filtered.drop(['time'], axis=1), 
                                             nbclusters)
    ...
    KMF_data = pd.DataFrame({"ratio": data.ratio, "sdn": data.sdn, 
                             "sde": data.sde, "sdu": data.sdu, 
                             "zsc_e": data.zsc_e, "ns": data.ns})
    optimal_nbclusters = elbow_point(KMF_data)
    print('Elbow point: done')
    data_clustered, centroids, labels = K_means(KMF_data, 
                                                nbclusters=optimal_nbclusters)
    ...
    if representation:
        # plot every points with the appropriate color, each one corresponding 
        # to a different cluster 
        # a color vector is created to be sure that similar colors are not 
        # used several times
        colors = ["green", "red", "blue", "orange", "grey", "black", 
        ...
        for i in counter:
            if projection == '2D':
                plt.scatter(data_clustered.X[data_clustered.label == i],
                            data_clustered.Y[data_clustered.label == i],
                            color=colors[i], alpha=0.5, label='cluster: %d' % i)
            elif projection == '3D':
                ax.scatter(data_clustered.X[data_clustered.label == i],
                           data_clustered.Y[data_clustered.label == i],
                           data_clustered.Z[data_clustered.label == i],
    ...
    if nb_cycles > 1:
        for i in range(0, nb_cycles - 1):
            data_filtered = KMF_correction(data_filtered, 
                                           nbclusters=optimal_nbclusters)
            data_filtered = appropriate_datetime(data_filtered, freq='24H')
            KMF_velocity = velocity(data_filtered)
            KMF_velocities.append(KMF_velocity)
    ...
    for i in range(0, nb_cycles):
        missval.append(np.sum(np.isnan(KMF_velocities[i])) 
                       / len(KMF_velocities[i]) * 100)
    
    nb_cycles_range = np.arange(1, nb_cycles + 1)
    kn = KneeLocator(nb_cycles_range, missval, curve='convex', 
                     direction='decreasing')
...
def EWS(df, w1=1.5, w2=0.5, t=5, representation=False, save=False):
    '''
    The Exponential Weighted Smoothing (EWS) is a method developed by 
    Helbing, J. (2005) that post-processes GNSS measurements (with a 
    timestep t in seconds) to obtain the resulting velocities. 
    ...
    def residuals_determination(df, wd=4, ts=5, linear='True'):
        ...
        res_y = df.Y - trend_y
        res_z = df.Z - trend_z
        res_xyz = pd.DataFrame({'X': res_x, 'Y': res_y, 'Z': res_z, 
                                'trend_x': trend_x, 'trend_y': trend_y,
                                'trend_z': trend_z, 'Q': df.Q, 'sde': df.sde,
                                'sdu': df.sdu, 'sdn': df.sdn, 
                                'global_sd': df.global_sd})
    ...
    EWS_velocity_interm = np.sqrt(np.diff(res_xyz_filt.X) ** 2 
                                  + np.diff(res_xyz_filt.Y) ** 2) 
                                  * ((3600 * 24) / 5)
...
def GWS(df, w=60, representation=False, save=False):
    '''
    The Gaussian Weighted Smoothing (GWS) is a method developed by 
    Sugiyama, S (2015) that post-processes GNSS measurements to obtain the 
    ...
    The window sizes and data timesteps are initially fixed to coincide 
    with S. Sugiyama's work.
    If representation=True, the resulting velocity is plotted.
    If save=True, the resulted velocities are saved in a csv file.
    '''
    ...
    GWS_velocity = (np.sqrt(np.diff(X_smoothed) ** 2 
                            + np.diff(Y_smoothed) ** 2) / 5) * 4 * 24
...
def KM2FA_DR(data, ratio_threshold=2, nbc_min=1, nbc_max=10, nbins=200, 
             standardisation=True, representation=False, save=False):
    ''' 
    KM2FA Daily Run.
    If standardisation=True, the variables are reduced and normalized. 
    If representation=True, features characteristics are displayed in the form 
    of histograms with a nbins number of bins.
    If save=True, features characteristics are saved in a csv file.
    '''
    
    def KM2FA(data, nbc_min=1, nbc_max=10, standardisation=True, 
              representation=False, save=False):
        ''' The K-Means Filtering for Features analysis (KM2FA) is a post-
        processing algorithm developed to describe clouds of points shifted 
        off a GNSS station track.
        It consists in three steps:
        1) Ratio variable filter 
        2) K-Means algorithm run
        3) Features characteristics determination
        The variables and ratio threshold are initially fixed to the one used 
        in Adrien Wehrlé's work.
        '''
        
        def elbow_point(data, mini=nbc_min, maxi=nbc_max):
            ''' 
            Determine the optimal number of clusters (optimal_nbclusters) 
            with the help of the Elbow Method. KMF algorithm is run with a range 
            of number of clusters (from nbc_min to nbc_max):
            when plotting the Sum of Squared Error (SSE) as function of the 
            number of clusters, the elbow of the curve is the optimal one.
            '''
        ...
        def features_characteristics(data_clustered, labels):
            '''
            Determination of the features characteristics in terms of duration 
            (features_duration), time of the day (mean_datetime), ratio variable 
            (mean_ratio), number of satellites (mean_ns), Q variable (mean_Q) 
            and number of values (nb_values). 
            '''
            ...
            for i in counter:
                ...
                dtime = np.array(data_interm.index.hour) 
                        + np.array(data_interm.index.minute) / 60  # in decimal hours
                ...
                feature_duration = (data_interm.index[-1] - data_interm.index[0]) \
                                    / timedelta(days=1)
                features_duration.append(feature_duration)
                
            features_duration = pd.DataFrame(features_duration)
            results = pd.DataFrame({"features_duration": features_duration.iloc[:, 0], 
    ...
    # uncomment if not already determined
    # data['global_sd'] = np.sqrt(data.sde ** 2 + data.sdu ** 2 + data.sdn ** 2) 
    features = data[data.ratio < 2]
    perc_features = len(features) / len(data)
    print('Percentage of features in raw data: %.3f' % perc_features)
    KM2FA_data = pd.DataFrame({"X": features.X, "Y": features.Y, 
                               "time": features.timestamp,
                               "global_sds": features.global_sd})
    
    features_characteristics = KM2FA_data.resample('1D').apply(KM2FA)
    if representation:
        legends = ['Features duration (hours)', 'Features mean time of the day (hours)',
                 'Features mean ratio ($\o$)', 'Features mean number of satellites ($\o$)',
                 'Features mean Q ($\o$)', 'Features mean number of values ($\o$)']
        
        plt.figure()
        plt.suptitle('Features characteristics', fontsize=21)
        for i in range(1, 7):
            plt.subplot(3, 3, i)
            plt.hist(features_characteristics.iloc[:, i - 1] * 24, bins=nbins,
                     color='orange')
