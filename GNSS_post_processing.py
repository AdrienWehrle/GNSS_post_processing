...
def KMF(
    ...
    def KMF_correction(data_filtered, nbclusters):
        ...
        data_filtered_interm, c, l = K_means(
            data_filtered.drop(["time"], axis=1), nbclusters
        )
        ...
        for i in range(0, len(mean_time_h)):
            if mean_time_h.iloc[i] / mean_time_h.iloc[i] == 1:
                ...
                if sampling_freq[-1] == "T":
                    ...
                    date = date.replace(
                        minute=int(mean_time_m.iloc[i] + df_modified.index.minute[i])
                    )
                dt.append(date)
            else:
                dt.append(
                    datetime.strptime(str(df_modified.index[i]), "%Y-%m-%d %H:%M:%S")
                )
        dt = pd.to_datetime(dt, format="%Y-%m-%d %H:%M:%S")
    ...
    if representation:
        ...
        for i in counter:
            if projection == "2D":
                plt.scatter(
                    data_clustered.X[data_clustered.label == i],
    ...
    for i in range(0, nb_cycles):
        missval.append(
            np.sum(np.isnan(KMF_velocities[i])) / len(KMF_velocities[i]) * 100
        )
...
def EWS(df, w1=1.5, w2=0.5, t=5, representation=False, save=False):
    ...
    EWS_velocity_interm = np.sqrt(
        np.diff(res_xyz_filt.X) ** 2 + np.diff(res_xyz_filt.Y) ** 2
    ) * ((3600 * 24) / 5)
...
def GWS(df, w=60, representation=False, save=False):
    ...
    GWS_velocity = (
        (np.sqrt(np.diff(X_smoothed) ** 2 + np.diff(Y_smoothed) ** 2) / 5) * 4 * 24
    )
...
def KM2FA_DR(
    ...
    def KM2FA(
        ...
        def elbow_point(data, mini=nbc_min, maxi=nbc_max):
            ...
            for k in K:
                ...
                distortions.append(
                    sum(
                        np.min(
                            cdist(data, kmeanModel.cluster_centers_, "euclidean"),
                            axis=1,
                        )
                    )
                    / data.shape[0]
        ...
        def features_characteristics(data_clustered, labels):
            ...
            for i in counter:
                ...
                dtime = (
                    np.array(data_interm.index.hour)
                    + np.array(data_interm.index.minute) / 60
                )  # in decimal hours
                ...
                feature_duration = (
                    data_interm.index[-1] - data_interm.index[0]
                ) / timedelta(days=1)
            ...
            results = pd.DataFrame(
                {
                    "features_duration": features_duration.iloc[:, 0],
    ...
    if representation:
        legends = [
            "Features duration (hours)",
            "Features mean time of the day (hours)",
            "Features mean ratio ($\o$)",
            "Features mean number of satellites ($\o$)",
            "Features mean Q ($\o$)",
            "Features mean number of values ($\o$)",
        ]
        ...
        for i in range(1, 7):
            plt.subplot(3, 3, i)
            plt.hist(
                features_characteristics.iloc[:, i - 1] * 24, bins=nbins, color="orange"
            )
