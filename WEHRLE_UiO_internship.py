...
def SDF_EM(df,sd_threshold=np.nanmean(df.global_sd),fq='24H',sampling_fq='1H',w_min=0,w_max=3*np.nanstd(df.global_sd),w_step=0.0001,temporal_resolution=False,representation=False,save=False):
    ...
    def SDF(df,sd_threshold=np.nanmean(df.global_sd),fq='24H',sampling_fq='1H',w_min=0,w_max=3*np.nanstd(df.global_sd)):
        ...
        def amb_filter(df,init=df_length):
            ...
            print('amb_filter: %d values removed' % nb_del_values, '(%.5f %% of the raw data)' % pc_add_values)
        ...
        def sd_filter(df,init=df_length,sdthr=sd_threshold):
            '''
            Deleting all values that have a global standard deviation (global_sd) above a given threshold (sdthr).
            The global standard deviation is obtained by combining the standard deviations of each positioning component 
            (X, Y and Z) computed by RTKLIB, at each timestep. 
            '''
            #df['global_sd']=np.sqrt(df.sde**2+df.sdu**2+df.sdn**2) uncomment if not already determined
            initial_len=len(df)
            df = df[df.global_sd<sdthr]
            final_len=len(df)
            nb_del_values=initial_len-final_len
            pc_add_values=nb_del_values/init*100
            print('sd_filter: %d values have been removed' % nb_del_values, '(%.5f %% of the raw data)' % pc_add_values)
        ...
        def pos_filter(df,init=df_length):
            ...
            nb_del_values=initial_len-final_len
            pc_add_values=nb_del_values/init*100
            print('pos_filter: %d values removed' % nb_del_values, '(%.5f %% of the raw data)' % pc_add_values)
        ...
        print('\n')
        print('In total, %d values removed' %nbdv_total,'(%.3f %% of the raw data)' % per_nbdv_total)
        data_filt_dt=appropriate_datetime(data_filt,freq=fq,sampling_freq=sampling_fq)
        
        #Velocity determination
        deltatime=np.diff(data_filt_dt.index)
        deltatime=pd.DataFrame(deltatime)
        deltatime_dec=deltatime.iloc[:,0]/timedelta(days=1)
        SDF_velocity=pd.DataFrame((np.sqrt(np.diff(data_filt_dt.X)**2+np.diff(data_filt_dt.Y)**2))/deltatime_dec)
    ...
    if temporal_resolution==True:
        ...
        for k in range(0,len(freqs)):
            ...
            for i in global_sd_range:
                SDF_velocity,SDF_vertvelocity=SDF(df,sd_threshold=i,fq=freqs[k][0],sampling_fq=freqs[k][1])
            ...
            if np.int(len(global_sd_range)/10)%2==0: #window size must be odd 
                regress=scipy.signal.savgol_filter(missval, np.int(len(global_sd_range)/10)+1, 1) 
            else:
                regress=scipy.signal.savgol_filter(missval, np.int(len(global_sd_range)/10), 1)
            kn = KneeLocator(global_sd_range[:j+1],regress,curve='convex', direction='decreasing') #improvment needed to automatically find the curve shape and slope
            optimal_sdthreshold=kn.knee
            print('KNEE:',optimal_sdthreshold)
            SDF_velocity,SDF_vertvelocity=SDF(df,sd_threshold=optimal_sdthreshold,fq=freqs[k][0],sampling_fq=freqs[k][1])
            ...
            if representation==True:
                ...
                plt.axvline(optimal_sdthreshold,label='knee point: %.3f' % optimal_sdthreshold,color=colors[k])
                plt.subplot(212)
                plt.plot(SDF_velocity.index,SDF_velocity.iloc[:,0],color='black',alpha=0.5)
        ...
        for i in global_sd_range:
            SDF_velocity,SDF_vertvelocity=SDF(df,sd_threshold=i,fq='24H',sampling_fq='1H')
            missval.append(np.sum(np.isnan(SDF_velocity))/len(SDF_velocity)*100)
            mv=np.float(np.sum(np.isnan(SDF_velocity))/len(SDF_velocity)*100)
            if mv==100:
                t=t+1
            ...
            if np.int(len(global_sd_range)/10)%2==0: #window size must be odd 
                ...
                regress=scipy.signal.savgol_filter(missval, np.int(len(global_sd_range)/10), 1)
            kn = KneeLocator(global_sd_range[:j+1],regress,curve='convex', direction='decreasing')
            optimal_sdthreshold=kn.knee
            print('KNEE:',optimal_sdthreshold)
            SDF_velocity,SDF_vertvelocity=SDF(df,sd_threshold=optimal_sdthreshold,fq='24H',sampling_fq='1H')
...
def RF_EM(df,ratio_threshold=np.nanmean(df.ratio),w_min=0,w_max=3*np.nanstd(df.ratio),w_step=0.5,temporal_resolution=False,representation=False,save=False):
    ''' Determination of the RF method's optimal threshold (step 1) with the help of the Elbow Method (from w_min to w_max each w_step) 
    to compute daily velocities (see Adrien Wehrlé's internship report).
    If temporal_resolution=True, velocities are computed for each temporal 
    resolutions (24, 12, 8, 6, 3 and 1 hour).
    If representation=True, results of the Elbow Method and velocities are displayed.
    ...
    def RF(df,ratio_threshold=np.nanmean(df.ratio),w_min=0,w_max=3*np.nanstd(df.ratio),w_step=0.5):
        ...
        def ratio_filter(df,ratiothr=ratio_threshold,init=df_length):
            '''
            Deleting all values that have a ratio value (global_sd) above a given threshold (ratiothr).
            The global standard deviation is obtained by combining the standard deviations of each positioning component 
            (X, Y and Z) computed by RTKLIB, at each timestep. 
            '''
            initial_len=len(df)
        ...
        deltatime=pd.DataFrame(deltatime)
        deltatime_dec=deltatime.iloc[:,0]/timedelta(days=1)
        RF_velocity=pd.DataFrame((np.sqrt(np.diff(data_filt_dt.X)**2+np.diff(data_filt_dt.Y)**2))/deltatime_dec)
    ...
    if temporal_resolution==True:
        ...
        for k in range(0,len(freqs)):
            ...
            for i in ratios_range:
                RF_velocity,RF_vertvelocity=RF(df,ratio_threshold=i,fq=freqs[k][0],sampling_fq=freqs[k][1])
            ...
            if np.int(len(ratios_range)/10)%2==0: #window size must be odd 
                regress=scipy.signal.savgol_filter(missval, np.int(len(ratios_range)/10)+1, 1)
            else:
                regress=scipy.signal.savgol_filter(missval, np.int(len(ratios_range)/10), 1)
            kn = KneeLocator(ratios_range[:j+1],regress,curve='convex', direction='increasing')
            optimal_rthreshold=kn.knee
            print('KNEE:',optimal_rthreshold)
            RF_velocity,RF_vertvelocity=RF(df,ratio_threshold=optimal_rthreshold,fq=freqs[k][0],sampling_fq=freqs[k][1])
            ...
            if representation==True:
                ...
                plt.ylabel('Missing values percentage ($\%$)',fontsize=14)
                plt.axvline(optimal_rthreshold,label='knee point: %.3f' % optimal_rthreshold,color=colors[k])
                plt.subplot(212)
                plt.plot(RF_velocity.index,RF_velocity.iloc[:,0],color='black',alpha=0.5)
                plt.xlabel('Time (year-month)',fontsize=16)
                plt.ylabel('Velocity (meters/day)',fontsize=16)
                print('%d' % k,'/ %d' % len(freqs))
                j=0
            ...
            for i in ratios_range:
                RF_velocity,RF_vertvelocity=RF(df,ratio_threshold=i,fq='24H',sampling_fq='1H')
                missval.append(np.sum(np.isnan(RF_velocity))/len(RF_velocity)*100)
                mv=np.float(np.sum(np.isnan(RF_velocity))/len(RF_velocity)*100)
                if mv==100:
                    t=t+1
                ...
                optimal_rthreshold=kn.knee
                print('KNEE:',optimal_rthreshold)
                SDF_velocity,SDF_vertvelocity=RF(df,ratio_threshold=optimal_rthreshold,fq='24H',sampling_fq='1H')
                results=pd.DataFrame([SDF_velocity,SDF_vertvelocity])
                ...
                if representation==True:
                    ...
                    plt.xlabel('Global standard deviation threshold',fontsize=14)
                    plt.ylabel('Missing values percentage ($\%$)',fontsize=14)
                    plt.axvline(optimal_rthreshold,label='knee point: %.3f' % optimal_rthreshold)
                    plt.subplot(212)
                    plt.plot(SDF_velocity.index,SDF_velocity.iloc[:,0],color='black',alpha=0.5)
                    plt.xlabel('Time (year-month)',fontsize=16)
                    plt.ylabel('Velocity (meters/day)',fontsize=16)
...
def KMF(df,nbc_min=1,nbc_max=10,nb_cycles=5,standardisation=False,variables_importance=False,representation=False,projection='2D',save=False):
    '''The K-Means filtering (KMF) is a method that post-processes GNSS measurements
        to obtain the resulting velocities. It is based on K-means automatic clustering, a basic machine
        learning algorithm that divides a dataset into k groups by determining the relations between 
        ...
        If representation=True, a scatter plot of the data with the clusters as colors is represented in 2D (projection='2D')
        or in 3D (projection='3D').
    ...
    def zscore(df):
        '''
        Computing the minimum distance (mindist) to the linear trend of the station track for each value,
        then z-score transformed.
        '''
